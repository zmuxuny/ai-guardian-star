...
- **分支约定（2026-08-15）**：`feature/jyx-home` 是本人（简沅晞，现任队长）的工作分支，`main` 是主分支。任何改动默认先在 `feature/jyx-home` 上做，完成后同步到 `main`；除非明确要求，不要新建分支。
...
- [2026-08-23] **versionCode 编码规则为 `1000000 + major*10000 + minor*100 + patch`。** 实测 0.1.0=1000000、0.2.0=1000200、0.2.1=1000201、1.0.0=1010000。AGC 要求 versionCode 严格递增，改版本号时必须按此公式算，不能凭 versionName 直觉取值。
- [2026-08-23] **版本策略：过审上架前版本号停留在 0.x，首个通过华为应用市场审核的版本才记为 1.0.0。** 该策略写在 README 与 README.zh-CN 的版本说明段落里，改版本时两个 README 都要同步。
