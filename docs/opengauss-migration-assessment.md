...
当前不切换生产数据库。服务器已有 openGauss 6.0.3 LTS，实例基本为空，可以在后续整改后复用；但现有 ECS 只有 2 核、约 3.3 GiB 内存且无 Swap，低于官方个人开发最低 2 核 4 GB，并且数据库缺少可靠 systemd 自启。现在迁移会降低登录系统可靠性。
...
- 安全整改：15400/15401 已从所有网卡收口到 IPv4/IPv6 回环地址；原配置备份保留在数据目录。
- 剩余风险：未找到正式的 `opengauss.service`，实例重启后的自动恢复能力不足。
...
3. 保持数据库仅监听回环地址，并继续收紧 `pg_hba.conf` 和华为云安全组。
