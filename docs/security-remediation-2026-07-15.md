# 智护星 App 安全评估整改方案（状态更新至 2026-07-17）

> 文档状态：整改基线 v1.1。2026-07-17 已同步当前代码与服务器核验结果；仍有上线阻断项，**不得据此宣称已完全整改或完全合规**。本文是工程整改方案，不替代律师意见、主管部门书面意见或平台审核结论。涉及“实名制”的最终适用范围，以公安、网信等主管部门针对本产品业务形态的明确要求为准。
>
...
### 3.1 截至 2026-07-17 的完成状态与阻断项

已完成并有当前代码或服务器证据支持的止血项：

- 公网 API 已由受信域名、Nginx 和 HTTPS 承载，证书自动续期已配置；客户端业务与 AI 请求不再回退 HTTP。
- 服务端代码已实现不透明短期 access token、轮换 refresh token 和 `Authorization: Bearer` 本人鉴权；资料更新不再信任客户端传入的账号标识，注册默认关闭，AI 与管理后台默认关闭，应用监听地址默认限制为 `127.0.0.1`。
- 服务端继续使用带随机盐的版本化 `scrypt` 作为过渡密码止血；登录响应、资料接口和管理后台不再返回、显示或通用修改密码字段。最终密码方案仍须迁移到压测后的 Argon2id。
- App 已改为仅在内存保存会话，统一附加 Bearer token，并对并发 401 使用单飞 refresh；已关闭本地认证、离线注册和云端失败后本地自动建号。
- App 数据库 schema 已升至 v5，升级时清除历史本地密码；云端请求日志不再打印请求体、响应体或异常明细。
- 邮箱页只允许已有账号登录，不再自动注册；仓库聚合快照和客户端共享 MQTT 密码已移除；默认任意来源 CORS 已关闭，服务端秘密改由环境注入。

2026-07-17 服务器部署与边界验证已完成：

- 部署前备份位于 `/root/wenxin-backups/20260717T183346Z`；`wenxin.service` 已处于 active、enabled，systemd 重启恢复验证通过。
- 应用仅监听 `127.0.0.1:8899`，公网 `8899` 已不可达；Nginx 到本机 `8899` 的内部反代继续保留，公网统一通过 HTTPS `443` 访问。
- HTTPS 验证结果：`/health` 返回 200，注册接口返回 503，管理后台路由返回 404，匿名 `/ai/chat` 返回 401。以上只证明本轮部署和默认关闭/鉴权边界生效，不代表后续未完成项已合规。

下列项目仍是生产上线阻断项，不能因上述止血完成而标记为“已整改”或“已合规”：

- 手机号注册尚无真实短信供应商和服务端 OTP 发送、核验、消费、重放防护；新注册必须继续保持关闭。
- AI 已具备 Bearer 鉴权且默认关闭，但仍缺用户/IP/设备限流、输入/输出内容审核、账号/AI 状态处置和上游熔断；长度限制不等于内容安全，风控完成前不得开启。
- refresh token 目前仅存内存，尚未迁入 HUKS；App 重启后需重新登录。ArkDB 敏感数据加密与备份收口也未完成。
- 服务端仍是过渡 `scrypt`，Argon2id 迁移尚未完成；账号治理、管理员 MFA/RBAC、审计、通知、人工复核和申诉闭环尚未建设。
- MQTT Broker 的 TLS 修复不等于设备迁移完成；香橙派仍待改用 TLS `8883` 和设备独立凭据，验证后才能关闭公网明文 `1883`。
- 仓库删除不等于从 Git 历史、旧安装包或服务器中吊销凭据；Coze、MQTT 等旧凭据仍须在对应平台轮换并验证旧值失效。
...
| 工作 | 当前状态 | 交付物 | 验收门槛 |
| --- | --- | --- | --- |
| 阻断未鉴权更新 | 已部署；完整越权回归仍按验收门槛执行 | 旧接口改为从 Bearer token 获取本人身份并限制字段 | 无 token 401；A 用户不能改 B 用户；普通资料接口不能改密码/手机 |
| 强制 TLS | API/AI 已完成；MQTT 设备迁移未完成 | ECS/API/AI/MQTT 全链路 TLS，删除 HTTP 回退 | 抓包无明文密码、token、手机号、聊天；错误证书请求失败 |
| 服务进程与端口收口 | 2026-07-17 已完成并验证 | systemd 托管仅监听 `127.0.0.1:8899`；公网仅开放 `443` | 重启自动恢复；公网无法直连 `8899`；HTTPS 健康检查通过 |
| 凭据处置 | 仓库/客户端止血完成；平台轮换验收未完成 | 吊销/轮换 Coze、MQTT 等凭据，迁入环境秘密/KMS；清理仓库快照与构建产物 | 旧凭据不可用；客户端包和仓库扫描无生产 secret |
| 关闭伪实名注册 | 已部署；注册接口返回 503 | 暂停邮箱注册、离线自动注册和跳过验证码路径；服务端默认关闭注册 | 无真实 OTP 时不能创建 `phone_verified` 账号 |
| 日志止血 | 已部署；完整日志抽检仍按验收门槛执行 | 删除完整请求/响应日志，管理后台隐藏密码列 | 日志/后台检索不到密码、验证码、token、完整聊天正文 |
| AI 止血 | 鉴权与默认关闭已完成；限流/审核未完成 | `/ai/chat` 必须鉴权、基础限流；高风险关键词/分类命中不转发 | 匿名请求 401；被拦截请求的上游 mock 调用为 0 |
| HarmonyOS 客户端构建 | 2026-07-18 已完成 | `targetSdkVersion` 升至 HarmonyOS API 23，最低兼容仍为 API 22；生成签名 HAP | Hvigor `assembleHap` 成功，产物完成签名 |

### Phase 1：1～2 周完成账号与数据基线

- 未完成：建立新数据模型、服务端 UUID 用户主键、唯一手机号盲索引和迁移脚本。
- 未完成：接入短信供应商，完成 OTP 发送/校验/重放/限流；邮箱改为登录后可选验证绑定。
- 部分完成：服务端已实现短期不透明 access token、轮换 refresh token 和会话撤销；Argon2id 仍未完成，当前 `scrypt` 仅为过渡止血。
- 部分完成：App 已删除本地密码登录和密码比较；云端登录成功后清除对应本地密码。历史仅本地账号的旧数据暂留待手机号迁移，但不再参与认证；refresh token 转入 HUKS、ArkDB 敏感库加密和备份收紧仍未完成。
- 未完成：手机变更、邮箱变更、忘记密码均改为专用安全流程。
- 部分完成：密码查看/通用编辑已移除；管理员 MFA、RBAC、审计日志仍未完成。

### Phase 2：1 周完成 AI 风控和账号治理

- 未完成：AI 网关加入状态校验、用户/IP/设备限流、输入/输出审核、上游熔断和成本控制；完成前 AI 保持默认关闭。
- 未完成：建立 `account_actions`、`moderation_events`、`appeals`，上线用户通知和申诉入口。
- 未完成：运营人员完成处置规则、复核 SLA、证据最小化和应急上报培训。
- 未完成：用真实照护语料建立误报集，重点验证疾病、自伤求助、照护讨论不被机械处罚。
