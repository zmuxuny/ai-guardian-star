...
### Fixed
- 1.0.1 修复包：登录后领取独立、短期、只读 MQTT 凭据，支持续期和注销撤销；保留外部 JSON 字段，修复 Release 混淆导致的反复重连。
- 开发板摄像头断连后自动恢复；增加 Ascend MPI 板载 3.5mm 音频输出。

### Changed
- 修复包版本为 `1.0.1` / `1010001`，目标与最低兼容 SDK 统一为 API 24；尚未发布应用市场。
